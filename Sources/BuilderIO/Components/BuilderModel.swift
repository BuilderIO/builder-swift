  ...
  var body: some View {
    ...
    ScrollView {
      ForEach(Array(blocks.enumerated()), id: \.offset) { index, child in
        ...
        let isHorizontal =
          (responsiveStyles["flexDirection"] == CSSConstants.FlexDirection.row.rawValue)
        let layout =
          isHorizontal
          ...
          if isHorizontal {
            ScrollView(.horizontal, showsIndicators: false) {
              layout {
                ...
                ResponsiveStylesBuilderView(
                  responsiveStyles: responsiveStyles ?? [:], isText: false))
            }
          } else {
            layout {
              layoutContent(for: child)
            }.modifier(
              ResponsiveStylesBuilderView(responsiveStyles: responsiveStyles ?? [:], isText: false))
          }
        }
