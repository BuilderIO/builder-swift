...
class CSSStyleUtil {
  ...
  static func getColor(value: String?) -> Color {
    if value != nil {
      ...
      let allMatches = matchingStrings(
        string: value!, regex: "rgba\\((\\d+),\\s*(\\d+),\\s*(\\d+),\\s*(\\d+)\\)")
      if allMatches.count > 0 {
        let matches = allMatches[0]

        if matches.count > 3 {
          return Color(
            red: Double(matches[1])! / 255, green: Double(matches[2])! / 255,
            blue: Double(matches[3])! / 255, opacity: Double(matches[4])!)
        }
      } else {
        if value?.hasPrefix("var") == true {
          return hexStringToUIColor(
            hex: extractHexValueFromVarString(from: value ?? "#fff") ?? "#fff")
        }
  ...
  static func hexStringToUIColor(hex: String) -> Color {
    ...
    if cString.count == 3 {
      ...
      cString =
        String(repeating: r, count: 2) + String(repeating: g, count: 2)
  ...
  static func getFloatValue(cssString: String?, defaultValue: CGFloat = CGFloat(0)) -> CGFloat {
    ...
    if let regex = try? NSRegularExpression(pattern: "px$") {  // TODO: handle decimals
      let newString = regex.stringByReplacingMatches(
        in: cssString ?? "", options: .withTransparentBounds,
        range: NSMakeRange(0, (cssString ?? "").count), withTemplate: "")
    ...
    if finalStyles[boxStyleProperty] != nil {
      ...
      for direction in directions {
        if finalStyles[boxStyleProperty + direction] != nil {
          return EdgeInsets(
            top: getFloatValue(cssString: finalStyles[boxStyleProperty + "Top"]),
  ...
  static func getHorizontalAlignmentFromMargin(styles: [String: String]) -> BuilderHorizontalAlignment {
    ...
    let isMarginLeftAbsentOrZero =
      marginLeft == nil || getFloatValue(cssString: marginLeft) == CGFloat(0)
    let isMarginRightAbsentOrZero =
      marginRight == nil || getFloatValue(cssString: marginRight) == CGFloat(0)
